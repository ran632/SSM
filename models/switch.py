...
class Switch(ndb.Model):
    ...
    @staticmethod
    def allRequests():
        qry = "SELECT * FROM Switch ORDER BY status DESC , date DESC "
        return ndb.gql(qry)


    @staticmethod
    def myRequests(empno):
        qry = "SELECT * FROM Switch WHERE from_empno = '%s' ORDER BY status ASC, date DESC" % (empno)
        return ndb.gql(qry)

    @staticmethod
    def recRequests(empno):
        qry = "SELECT * FROM Switch WHERE to_empno = '%s' ORDER BY status ASC, date DESC" % (empno)
        return ndb.gql(qry)
